...
exports.getAvgRating = function(req, res, next) {
  if (req.query.userID != "") {
    ...
    var queryToGo = "SELECT AVG(rating) as rating, to_char(date, 'Mon-YYYY') as month From Skin.Entry Where userid=" + id1 + "Group  by userId, month ORDER BY userId, month DESC";
    client.query(queryToGo, function(err, qres) {
      if(err) {
